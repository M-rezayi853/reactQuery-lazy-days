  ...
  const { mutate: patchUser } = useMutation(
    (newUserData: User) => patchUserOnServer(newUserData, user),
    {
      ...
      onMutate: async (newData: User | null) => {
        // cancel any outgoing queries for user data, so old server data doesn't
      ...
      onError: (error, newData, preUserDataContext) => {
        // roll back cache to saved value
        if (preUserDataContext.previousUserData) {
          updateUser(preUserDataContext.previousUserData);
          toast({
